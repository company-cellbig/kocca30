...
이 스키마는 [[CONVENTIONS#3.2 프론트매터 (YAML)]]의 변환 산출물 예외임. 변환 본문은 `type`, `status`를 넣지 않고 `source`, `converted`로 원본 경로와 변환일을 남김. DOCX이면 `tags`의 마지막 항목을 `docx`로 바꿈. 검수 큐 사이드카는 같은 예외를 적용하되 `title`, `tags`, `source`, `created`, `updated`만 사용함.
...
3. **원본 충실 전사**: 원문을 옮긴 영역은 의미가 있는 가운뎃점, em dash, 괄호와 특수문자를 보존함. 시각 마커처럼 구조 표지로 판정한 문자는 이 문서 6절과 9절에 따라 마크다운 구조로 바꿈. 작성자가 덧붙이는 설명은 [[CONVENTIONS#3.4 문체]]를 적용함
