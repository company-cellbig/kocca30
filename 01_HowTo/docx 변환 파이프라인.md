...
> 한컴 HWPX 또는 Word DOCX를 원본 추적이 가능한 마크다운으로 변환하는 표준 절차임. HWPX 직접 변환을 권장하며, 변환 본문, 원본, 검수 자산의 관계와 파서 정책 및 검수 방법을 정함.
...
폴더의 일반 역할과 수정 권한은 [[CONVENTIONS#2. 디렉토리 구조]]와 [[CONVENTIONS#6. 수정 금지 영역]]이 소유함. 파이프라인이 직접 읽거나 만드는 경로는 다음과 같음.

```
02_References/
├── _sources/<name>.<ext>           (입력 원본 사본)
├── _figures/<name>/                (추출 이미지)
│   ├── image1.png
│   └── ...
├── _reviews/<name>.review.md       (검수 큐 사이드카)
└── converted/<name>.md             (변환 본문)
```

- 다른 위치의 입력 파일은 `_sources/`에 복사함. 같은 이름의 원본이 이미 있으면 SHA-256이 같은 파일은 복사를 건너뛰고, 내용이 다르면 덮어쓰지 않고 실패함
- 재실행 때 기존 파일을 보호하는 동일성 검사는 `_sources/` 원본에만 적용함
	- `converted/<name>.md`와 `_reviews/<name>.review.md`는 새 결과로 덮어씀
	- `_figures/<name>/`의 같은 이름 이미지는 덮어쓰고, 새 변환에 없는 기존 파일은 자동으로 지우지 않음
	- 본문을 `converted/`의 하위 폴더로 옮긴 뒤 재실행하면 루트에 같은 이름의 새 본문이 생김. 하위 폴더의 기존 본문과 루트의 새 본문을 대조해 처리함
- HWPX 최초 출력의 본문 이미지 경로는 `![](../_figures/<name>/imageN.<ext>)`임. DOCX는 Pandoc이 만든 `media/` 하위 경로를 유지해 `![](../_figures/<name>/media/...)`로 출력함
- 변환 본문을 `converted/`의 하위 폴더로 옮기면 새 위치를 기준으로 이미지 상대경로를 고치고 대상 파일이 있는지 확인함
- 사이드카 wikilink는 위치와 무관하게 해석되는 `[[<name>.review]]`임
...
```
/import-doc "02_References/_sources/제안서.hwpx"
```
...
- 공통 자동 통계(H1/H2 헤딩, 표, 이미지)와 HWPX 파서의 세부 통계
- 변환기 경고가 있으면 그대로 표시

변환 뒤 MOC 등록, 로그 기록과 관련 문서 갱신은 [[위키 운영 워크플로#1. 수집 (Ingest): 소스 1개의 연쇄 갱신]]과 [[위키 운영 워크플로#5. 공통 문서 생성과 갱신 절차]]를 따름.

# 6. HWPX 파싱 정책 (V2 파서)

## 6.1 Section 처리 (다중 section 지원)
...
| `^[가-하]\.\s` | `##` (H2): 예: "가. 결과물" (단독 한글 라벨, 한컴 원본에서 같은 위계) |
...
불릿으로 처리하는 paragraph와 셀 안 paragraph의 줄머리에 붙은 시각 마커는 한컴 원본의 시각적 들여쓰기 표지임. 제거 대상은 다음과 같음.

- 제거 패턴: `^\s*(?:[○●◇◆■□▶▸▣•·ㆍ]|-)\s+`
- 적용: `handle_paragraph()`의 강제 1단 불릿과 `paraPr` 불릿 분기, `extract_cells()`의 셀 안 paragraph
- 줄머리에서 뒤에 공백이 있는 `·`, `ㆍ`, `-`도 구조 표지로 보아 제거함. 텍스트 내부의 기호는 보존함. 줄머리에 있더라도 뒤에 공백이 없는 기호는 의미가 있을 수 있어 보존함
- 불릿이 아닌 일반 paragraph는 이 제거 함수를 거치지 않으므로 줄머리 기호를 보존함
...
이 스키마는 [[CONVENTIONS#3.2 프론트매터 (YAML)]]의 변환 산출물 예외임. 위 양식은 파이프라인이 자동 생성한 HWPX와 DOCX 변환 본문에 적용함. 변환 본문은 `type`, `status`를 넣지 않고 `source`, `converted`로 원본 경로와 변환일을 남김. DOCX이면 `tags`의 마지막 항목을 `docx`로 바꿈.

사람이 스캔 이미지에서 옮긴 기존 전사본에도 같은 예외를 적용함. 이 경우 `tags`에 종목과 주제 태그를 더하고 `source`에 원본 이미지 폴더를 적을 수 있음.

검수 큐 사이드카는 `title`, `tags`, `source`, `created`, `updated`만 사용함.
...
3. **원본 충실 전사**: 원문을 옮긴 영역은 의미가 있는 가운뎃점, em dash, 괄호와 특수문자를 보존함. 시각 마커처럼 구조 표지로 판정한 문자는 [[#6.6 시각 마커 처리]]에 따라 마크다운 구조로 바꿈. 작성자가 덧붙이는 설명은 [[CONVENTIONS#3.4 문체]]를 적용함
4. **시각 콘텐츠는 파일 보존만**: 도표/차트는 figures/로 추출, 자동 해석 금지
5. **변환물은 정본 아님**: `converted/`는 사본. 인용 기준은 `_sources/`/`_locked/`
6. **추출 단계에서 메타데이터 폐기 금지**: 텍스트 옆의 구조 정보(스타일, 들여쓰기, 정렬, 셀 병합 등)는 출력 단계에서 안 쓰더라도 추출 결과에 보존. 추후 단순화는 출력 단계 책임. 한 번 폐기된 메타데이터는 복원 불가 ([[#8.8 패턴 8: 표 셀 병합 무시로 row 한 칸씩 밀림 (인용 신뢰성 fatal)]] 사례)
7. **AI 인용 안전성**: 변환물의 한 줄을 떼어내 다른 문서에 붙여넣어도 의미가 살아 있도록 self-contained 구조 우선. cross-product 평탄화([[#8.7 패턴 7: bullet block 안 pipe_table 미렌더 + AI 인용 안전성]]), 변환 한계 메모([[#8.9 패턴 9: 의미 보존 불가능한 표(월별 그리드 등) 자동 메모 처리]]), hierarchy 보존([[#8.10 패턴 10: 셀 안 paragraph hierarchy 손실 (paraPr 메타데이터 폐기)]]) 모두 같은 원리에서 도출
8. **fail-safe 휴리스틱**: 문서마다 다른 구조적 가정(헤딩 패턴 등)은 가정 실패 시 정보 손실보다 약간의 노이즈가 낫다 ([[#8.6 패턴 6: 헤딩 패턴 부재 문서에서 본문 전체 삼킴]] 사례)

# 8. 문제 패턴 사전

같은 류의 변환 작업에서 재발할 수 있는 패턴들. 진단/대처 기록.

## 8.1 패턴 1: 헤딩 손실 (한컴 export 경유 시)

- **증상**: H1/H2가 모두 `**볼드 단락**`으로만 나옴. `# 1-1.` 형태가 없음
- **원인**: 한컴 HWPX→DOCX export 시 paragraph 스타일이 Word `Heading 1/2`로 매핑 안 됨. Pandoc은 DOCX heading 스타일에 의존하므로 복원 불가
- **대처**: HWPX 직접 파싱 (V2 경로). DOCX를 써야 하면 `N-M-K.` 볼드 단락을 H2로 먼저 바꾸고, 남은 `N-M.` 볼드 단락을 H1으로 수동 보정함. 한컴이 부여한 볼드 패턴은 문서마다 다를 수 있어 자동 복원을 보장하지 않음

## 8.2 패턴 2: grid_table 정렬 깨짐 + raw text 표시 (Obsidian 등)
...
## 8.3 패턴 3: list block 끊김 (들여쓰기 bullet이 raw text로 표시)
...
- **대처**: list block 상태 추적(`last_bullet_indent` 등).
  ...
  - **표**: [[#8.7 패턴 7: bullet block 안 pipe_table 미렌더 + AI 인용 안전성]] 참고 (들여쓰기된 pipe_table은 렌더 안 되므로 별도 처리)
  - **헤딩/평문 paragraph**: list block 명시적 끊음(last_bullet_indent = -1): 의미적 단위 분리가 필요한 경우만

## 8.4 패턴 4: "라벨 ↔ 긴 내용" 표가 grid에 욱여넣어짐
...
- **증상**: ○를 H2 헤딩으로 만들면 그 아래 자식 bullet들이 paraPr 기반 들여쓰기(2/3단)로 시작 → H2 직후 부모 없는 들여쓰기 bullet으로 깨짐
- **원인**: 원본 한컴에서 ○가 헤딩이 아니라 시각적 1단 라벨로 쓰임. 자식 paragraph들의 paraPr idRef가 1단(`○ 자체`)을 건너뛰고 2단부터 시작
- **대처**: ○/●/◇/■ 시작 paragraph는 paraPr와 무관하게 강제 1단 bullet으로 바꾸고 줄머리 마크는 제거함

## 8.6 패턴 6: 헤딩 패턴 부재 문서에서 본문 전체 삼킴

- **증상**: 변환 결과 통계 모두 0 (H1/H2/표/이미지/bullets 전부). MD 본문이 frontmatter 외 비어 있음
- **원인**: 챕터 제목 흡수 휴리스틱(첫 H1을 만날 때까지 paragraph를 frontmatter title로 흡수)이 문서별 헤딩 패턴 차이를 가정 못 함. `N-M.` 패턴 H1이 없는 문서(예: 4.hwpx: 마일스톤 표 모음, `1 단계(1차연도)` 같은 다른 패턴)는 게이트가 영원히 열리지 않아 본문 281개 paragraph 전체가 삼켜짐
...
## 8.7 패턴 7: bullet block 안 pipe_table 미렌더 + AI 인용 안전성
...
## 8.8 패턴 8: 표 셀 병합 무시로 row 한 칸씩 밀림 (인용 신뢰성 fatal)
...
## 8.9 패턴 9: 의미 보존 불가능한 표(월별 그리드 등) 자동 메모 처리
...
## 8.10 패턴 10: 셀 안 paragraph hierarchy 손실 (paraPr 메타데이터 폐기)
...
## 8.11 패턴 12: 다중 section HWPX silent data loss
...
## 8.12 패턴 13: 변환 파이프라인이 _sources 원본을 silent overwrite

- **증상**: 같은 basename의 다른 파일을 import하면 기존 _sources 원본이 덮어쓰여 provenance 손실. 변환 성공으로 끝나서 발견이 어려움
- **원인**: `copyFileSync`는 기본 덮어쓰기. `_sources/`가 [[CONVENTIONS#6. 수정 금지 영역]]에서 추가만 허용하는 읽기 전용 영역인데 코드가 이 정책을 강제하지 않았음
...
## 8.13 패턴 11: 섹션 라벨 1행 표가 직전 bullet 깊이를 잘못 상속
...
`02_References/_reviews/<name>.review.md` 파일의 체크리스트를 항목별로 확인함.

- **헤딩 구조**: H1이 원본의 `N-M.` 절과, H2가 `N-M-K.` 하위 절과 대응되는지, 누락된 헤딩이 없는지
- **표**: 모든 표가 원본과 셀 내용 일치, 셀 병합 의미 보존
- **이미지**: 본문 참조 위치 정확, 캡션 보존
- **본문 일관성**: 페이지/문단 누락, 각주, 한자/특수문자 인코딩

문제가 없으면 사용자가 사이드카를 삭제함. 손상된 항목이 있으면 작성자가 `converted/<name>.md`를 고치고 [[위키 운영 워크플로#5.2 문서 갱신 절차]]를 수행함. `_reviews/`는 에이전트에게 읽기 전용인 보조 영역이므로 사이드카의 체크와 수정 내역 기록 및 삭제는 사용자가 맡음.

# 12. 트러블슈팅

## 12.1 `python을 찾을 수 없음` (HWPX 경로)

- `python --version` 또는 `python3 --version` 으로 확인
- Windows는 Microsoft Store에서 Python 설치하거나 `winget install Python.Python.3.12`

## 12.2 `pandoc을 찾을 수 없음` (DOCX 경로)
...
- 따옴표로 감쌀 것: `/import-doc "02_References/_sources/연구계획서_v2.hwpx"`
...
## 12.5 헤딩이 잡히지 않음 (HWPX)

V2 파서는 `\d+-\d+\.` 패턴에 의존. 원본이 "제1장", "I." 등 다른 번호 체계를 쓰면 매칭 안 됨. 필요 시 `assets/extract_hwpx.py`의 `H1_PAT`과 `H2_PAT` 정규식을 케이스별로 확장함.

# 13. 관련 문서

- [[CONVENTIONS#2. 디렉토리 구조]] / [[CONVENTIONS#6. 수정 금지 영역]]
- [[AGENTS#2.6 수정 금지 영역 준수]]
