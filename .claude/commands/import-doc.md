...
3. **결과 파싱 및 후속 작업**
   ...
   - JSON을 파싱하여 사용자에게 다음을 안내:
     ...
     - 헤딩/표/이미지 통계
     - 경고가 있다면 그대로 표시
   - 변환본이 무형유산 종목 소재 전사본이면 `converted/` 루트에서 종류별 하위 폴더(예: `converted/남사당놀이/`)로 옮김. 파이프라인은 항상 루트로 출력하므로 사람이 수동 이동함 (규약: [[CONVENTIONS#2. 디렉토리 구조]] converted 하위 폴더 분리). wikilink는 파일명 해석이라 이동해도 안 깨지지만, 본문 이미지의 상대경로는 새 위치에 맞게 고치고 대상 파일이 있는지 확인함
   - `[[위키 운영 워크플로#1. 수집 (Ingest): 소스 1개의 연쇄 갱신]]`과 `[[위키 운영 워크플로#5. 공통 문서 생성과 갱신 절차]]`에 따라 MOC 등록, 관련 문서 갱신과 로그 기록을 수행
...
- 같은 이름의 파일이 `_sources/`에 이미 있으면 SHA-256을 비교한다. 내용이 같으면 복사를 건너뛰고, 다르면 기존 원본을 덮어쓰지 않고 실패함
...
- 폴더 규약: [[CONVENTIONS#2. 디렉토리 구조]], [[CONVENTIONS#6. 수정 금지 영역]]
