...
stats에 `sections` 수 보고. section0만 하드코딩하면 multi-section 문서에서 section1+ 콘텐츠가 silent data loss됨 (Codex 검수 지적, 패턴 12 참고).
...
`^[○●◇■]\s` 패턴은 헤딩이 아니라 **1단 bullet**으로 처리 (이유는 §문제 패턴 사전 참고).
...
6. **추출 단계에서 메타데이터 폐기 금지** — 텍스트 옆의 구조 정보(스타일, 들여쓰기, 정렬, 셀 병합 등)는 출력 단계에서 안 쓰더라도 추출 결과에 보존. 추후 단순화는 출력 단계 책임. 한 번 폐기된 메타데이터는 복원 불가 (패턴 8/10 사례)
7. **AI 인용 안전성** — 변환물의 한 줄을 떼어내 다른 문서에 붙여넣어도 의미가 살아 있도록 self-contained 구조 우선. cross-product 평탄화(패턴 7), 변환 한계 메모(패턴 9), hierarchy 보존(패턴 10) 모두 같은 원리에서 도출
8. **fail-safe 휴리스틱** — 문서마다 다른 구조적 가정(헤딩 패턴 등)은 가정 실패 시 정보 손실보다 약간의 노이즈가 낫다 (패턴 6 사례)

## 문제 패턴 사전

같은 류의 변환 작업에서 재발할 수 있는 패턴들. 진단·대처 기록.

### 패턴 1: 헤딩 손실 (한컴 export 경유 시)
...
### 패턴 2: grid_table 정렬 깨짐 + raw text 표시 (Obsidian 등)
...
### 패턴 3: list block 끊김 (들여쓰기 bullet이 raw text로 표시)
...
### 패턴 7: bullet block 안 pipe_table 미렌더 + AI 인용 안전성
...
### 패턴 8: 표 셀 병합 무시로 row 한 칸씩 밀림 (인용 신뢰성 fatal)
...
### 패턴 9: 의미 보존 불가능한 표(월별 그리드 등) 자동 메모 처리
...
### 패턴 10: 셀 안 paragraph hierarchy 손실 (paraPr 메타데이터 폐기)
...
### 패턴 12: 다중 section HWPX silent data loss
...
### 패턴 13: 변환 파이프라인이 _sources 원본을 silent overwrite
...
### 패턴 11: 섹션 라벨 1행 표가 직전 bullet 깊이를 잘못 상속
