...
- **도달성과 입력 수단 (확인 필요, 미결)**: 센서 막대 모션 화면이라 시뮬 뷰는 화면을 크게 쓰되 대거리 선택지 터치는 중앙 도달 존에 둠. 센서 막대를 쥔 채 선택지를 고르는 물리 조작 모델은 기획서 미결이라 설계 단계에서 확정함: 한 손으로 막대를 잡고 반대 손으로 터치, 막대를 잠깐 내려놓고 터치, 또는 핸드 트래킹으로 손짓 선택 중 택. 어느 쪽이든 선택 요소는 도달 존에 둠([[버나 - 버나잡이 한 판 기획서]] §8 미결 대거리 입력 수단)
