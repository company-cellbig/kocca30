...
| 바 | 변환 대기 화면 | Step 3 | 공용 (대기, §5.다) | [[덧뵈기 - 나만의 탈춤 기획서#다. Step 3. 3D 변환 + 대기 화면 (병행)]] |
| 사 | AR 춤사위 체험 화면 | Step 4 | 덧뵈기 고유 (핵심) | [[덧뵈기 - 나만의 탈춤 기획서#라. Step 4. AR 착용 + 춤사위 따라하기 (핵심 체험)]] |
| 아 | 기념 촬영 화면 | Step 5 | 덧뵈기 고유 | [[덧뵈기 - 나만의 탈춤 기획서#마. Step 5. 기념 촬영 + 생성 (병행)]] |
| 자 | 생성 대기 화면 | Step 5 | 공용 (대기, §5.다) | [[덧뵈기 - 나만의 탈춤 기획서#마. Step 5. 기념 촬영 + 생성 (병행)]] |
| 차 | 결과물 미리보기 화면 | Step 6 | 덧뵈기 고유 | [[덧뵈기 - 나만의 탈춤 기획서#바. Step 6. 결과물 전송]] |
| 카 | 결과물 QR 화면 | Step 6 | 공용 (전송, §5.라) | [[덧뵈기 - 나만의 탈춤 기획서#바. Step 6. 결과물 전송]] |
...
- **덧뵈기 공통 요소**: 진행 호스트는 말뚝이임(역할은 [[덧뵈기 - 나만의 탈춤 기획서#자. 말뚝이 진행 호스트]], 구체 비주얼 미확정이라 전 화면에서 "말뚝이 탈 호스트 캐릭터"로만 묘사). 공통 화면에서는 말뚝이를 중앙 조작 존 옆 여유 공간에 또렷한 안내 캐릭터로 두고 유저를 향해 안내하는 포즈로 함(실사 행인이나 다른 사용자로 오인되지 않게, §5.가 호스트 슬롯). 본문이 화면 전체를 쓰는 고유 화면(탈 디자인 캔버스, AR 카메라 뷰, 촬영)에서만 작게 두거나 숨김
...
덧뵈기 고유 화면. 사용자가 만든 탈을 쓴 2D 탈 캐릭터가 탈춤 포즈를 취한 옆에서, 맨 얼굴의 사용자가 같이 포즈를 잡아 기념 사진을 촬영함. 탈을 쓴 쪽은 캐릭터뿐이고 사용자는 맨 얼굴이라 둘이 구별됨. 촬영은 버튼 없이 컷마다 주어진 시간이 카운트다운돼 0이 되면 자동으로 찍히고(찰칵), 정해진 촬영 횟수만큼 반복함 (기획서 "카운트다운 촬영", [[덧뵈기 - 나만의 탈춤 기획서#마. Step 5. 기념 촬영 + 생성 (병행)]]).
