...
- **레이아웃**: [[시범콘텐츠 공통 사양#5.2 인트로 (시작 + 동의)]] 시작 화면 준용. 헤더에 콘텐츠 타이틀, 본문에 유람기 대표 비주얼, 액션에 큰 시작 버튼을 두며, 헤더 한쪽 모서리에 언어 선택 요소를 둠(고른 언어가 세션 전체에 적용됨). 박첨지가 환영하며 등장함 (보조 이어하기 버튼은 2차연도 전방 설계라 1차연도 미배치, [[#2.1 화면 목록과 분류]] 나. 학습 토큰 스캔)
...
- **상태별 안내 (기획서 준용)**: 그만두기 버튼 터치 시 그만두기 확인 팝업([[시범콘텐츠 공통 사양#5.8 그만두기 확인 팝업]])을 거쳐 확정 시 미동의로 간주해 종료 Step 직행, 터치 미입력 타임아웃은 팝업 없이 미동의 종료. 기획서 Step 1과 [[시범콘텐츠 공통 사양#5. 공통 화면]]나를 준용함(목업은 정상 동의 상태만이라 확인 팝업 목업은 별도)
