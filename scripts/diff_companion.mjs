...
//   그대로인 절을 "확인 요구" 목록으로 출력함. 위키 운영 워크플로 5.2 문서 갱신 절차
//   5번(파생 텍스트 점검)의 기계 프롬프트임.
//
// 알림이지 판정이 아님:
...
//
// 제외:
//   99_Logs/(이력), 04_Projects/_archive/(보존), 02_References/(변환 산출물)
...
const SKIP = [/^99_Logs\//, /^04_Projects\/_archive\//, /^02_References\//];
const report = [];
...
if (report.length) {
	...
	console.log("  (위키 운영 워크플로 5.2 문서 갱신 절차 5번. 알림이지 반려가 아님: 대부분은 바꿀 것이 없는 게 정상임)");
